...
- Surface design decisions before making them. If a prompt's instruction proves brittle, ambiguous, or suboptimal during implementation, pause and surface the trade-off before deviating. Format: "Issue: <what's wrong with the literal instruction>. Options: <2–3 alternatives>. Recommendation: <which and why>. Confirm before I proceed?" Apply this to any non-trivial deviation — test shape, fixture structure, schema choice, library choice, API contract. Trivial implementation details (variable names, internal helper structure) do not need approval. When in doubt, ask.
